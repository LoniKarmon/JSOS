...
use crate::extensions::extension_data::alpn::{AlpnProtocolList, ProtocolName};
    ...
    pub(crate) fn encode(&self, buf: &mut CryptoBuffer<'_>) -> Result<(), TlsError> {
        ...
        buf.with_u16_length(|buf| {
            ...
            if let Some(ref protocols) = self.config.alpn_protocols {
                let mut names: heapless::Vec<ProtocolName, 4> = heapless::Vec::new();
                for p in protocols.iter() {
                    let _ = names.push(ProtocolName { name: p });
                }
                ClientHelloExtension::ApplicationLayerProtocolNegotiation(
                    AlpnProtocolList { protocols: names }
                ).encode(buf)?;
            }
